...
ECGame.EngineLib.EntityComponent_2DPhysics = ECGame.EngineLib.Class.create({
	Constructor : function EntityComponent_2DPhysics()
	{
		...
		this._boundingRect = ECGame.EngineLib.AABB2.create(this._position.myX, this._position.myY, 64, 64);//TODO separate position from AABB in PhysicsSim2D
		this._range = ECGame.EngineLib.AABB2.create(0, 0, 65535, 65535);//TODO used? Set when added to world?
	},
	...
	Definition :
	{
		...
		[
			...
			},
			{
				...
				max : ECGame.EngineLib.Point2.create(512,512)	//TODO replace hack numbers
			}
			//TODO rect NOT net!
		],
...
//			this._range = inEvent.world.getBoundingBox();
		...
		onRemovedFromWorld : function onRemovedFromWorld(inEvent)
		{
			...
			//this._range = ECGame.EngineLib.AABB2.create(0, 0, 256, 256);//TODO clamp values in range (in serializer?)
			//TODO clear position?
		},
